...
- [ ] C4. 從非 WebView 區域起始的返回手勢行為不變 — **未驗**（該路徑本就不經過本修正，
      但未實際確認）
- [~] C5. 與 `keyboardAvoidance` 的交互 — **部分完成**。兩個功能已於 2026-08-12 合併進 `main`
      （`3eca2c09a`），首次在同一建置中共存。
      - [x] **C5-a**：兩者並存下軟鍵盤避讓仍正常——焦點欄位完整浮於鍵盤上方，
        `physBottom=2065` 算出的間距仍為 22px。以合成注入驗證
      - [ ] **C5-b（未驗）**：**鍵盤開著時做邊緣返回手勢**。這是兩者唯一交會的路徑——
        手勢修正會丟棄 `MOVE`，同時 IME 動畫回呼正逐幀更新鍵盤高度、位移邏輯跟著調整。
        需真手指操作，合成注入無效。使用端回報「`avoid=true` + `resize=false` 最流暢」，
        但未確認是否涵蓋此特定動作，故**不視為通過**
